...
into a graph of runtime values that it reduces on demand. A bytecode backend
is designed as a second execution path sharing the same runtime; see
[5.Bytecode compiler](5.Bytecode%20compiler.md) for its architecture.
