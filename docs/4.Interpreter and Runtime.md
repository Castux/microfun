...
built by `WrapBinop`, which returns a `RuntimePartial` (capturing the first
argument) that applies the actual operation when the second argument arrives;
`WrapMonop` is the unary analogue. Both force their numeric arguments to weak
head normal form and type-check them, reporting a non-number as a clean runtime
error. The exact argument order of the arithmetic and comparison builtins — tuned
for partial application — is documented in
