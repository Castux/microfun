...
GOOS=js GOARCH=wasm go build -trimpath -ldflags="-s -w" -o "$out/microfun.wasm" .
...
cp web/index.html web/playground.html web/style.css \
   web/site.js web/playground.js web/runner.js web/worker.js web/microfun-mode.js \
   "$out/"

# The markdown the site renders, and the example programs the playground loads.
cp README.md LICENSE.md "$out/"
cp docs/*.md "$out/docs/"
cp examples/*.mf "$out/examples/"
