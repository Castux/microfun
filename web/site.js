...
// microfun code blocks into editable, runnable snippets backed by the wasm
// runner. index.html?page=<id> selects a page.

"use strict";

const PAGES = [
    { id: "home", title: "microfun", file: "README.md" },
...
// `mf`, `microfun`) becomes an editable, runnable snippet; `mf-static` is
// microfun that is a fragment rather than a program, shown highlighted but
...
function makeSnippet(pre, code, runnable) {
    ...
    runBtn.addEventListener("click", async () => {
        ...
        let got = "";
        const result = await MicrofunRunner.run(src, {
            path: "snippet.mf",
...
async function main() {
    ...
    document.title = page.id === "home" ? "microfun" : page.title + " — microfun";
    buildSidebar(page);
