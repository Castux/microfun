// Headless smoke test for the wasm build: runs a .mf program under Node the
...
//
//   node web/smoke.mjs <sitedir> <program.mf> [stdin-text] [dump-stage]
...
if (!siteDir || !programPath) {
    console.error("usage: node web/smoke.mjs <sitedir> <program.mf> [stdin-text]");
    process.exit(2);
}
...
globalThis.__microfun_source = readFileSync(programPath, "utf8");
globalThis.__microfun_path = programPath;
globalThis.__microfun_dump = dumpStage || "";

const go = new Go();
let exitCode = 0;
go.exit = code => { exitCode = code; };

const wasm = readFileSync(join(siteDir, "microfun.wasm"));
