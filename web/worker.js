// Web Worker that hosts the microfun wasm module. One worker handles runs
...
function getModule() {
    if (!modulePromise) {
        modulePromise = fetch("microfun.wasm").then(resp => {
            if (!resp.ok) throw new Error("could not fetch microfun.wasm (" + resp.status + ")");
...
onmessage = async event => {
    ...
    try {
        ...
        globalThis.__microfun_source = source;
        globalThis.__microfun_path = path || "playground.mf";
        globalThis.__microfun_dump = dump || "";
